...
const UpdateStudent = () => {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      try {
        ...
        setGuardianDetails(response.data.guardianDetails || {});
        setGuardianType(response.data.guardianType || 'mom');
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:5000/api/users/${studentId}`, formData);
      console.log('Response:', response.data);
      
      toast.success('Student Updated Successfully!');
      history.push('/user-management'); // Redirect to user management page after update
    } catch (error) {
